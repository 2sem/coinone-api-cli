...
import { maintenanceStatusLabel, marketPair, renderKeyValueTable, renderTable, tradeStatusLabel } from '../lib/formatters.js';
...
export function createMarketsCommand(client, emitResult) {
    ...
    command
        ...
        const response = await client.getMarket(normalizeQuote(options.quote), normalizeCode(targetCurrency));
...
function buildMarketsListResult(markets, raw) {
...
function buildMarketGetResult(market, raw) {
    const normalized = {
        pair: marketPair(String(market.target_currency), String(market.quote_currency)),
        quote: String(market.quote_currency).toUpperCase(),
        target: String(market.target_currency).toUpperCase(),
