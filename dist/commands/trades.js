...
export function createTradesCommand(client, emitResult) {
    ...
    command
        ...
        const response = await client.listTrades(options.quote.toLowerCase(), targetCurrency.toLowerCase(), options.size);
        emitResult(this, buildTradesResult(response, response));
    });
    return command;
}
function buildTradesResult(response, raw) {
